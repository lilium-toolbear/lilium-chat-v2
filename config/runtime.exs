...
#
#     PHX_SERVER=true bin/lilium_chat start
#
...
if config_env() == :prod do
  ...
  config :lilium_chat, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")

  # ToolBear JWT secret (spec §6.1) — same secret as the old Worker.
  jwt_secret =
    System.get_env("JWT_SECRET") ||
      raise("""
      environment variable JWT_SECRET is missing.
      Reuse the ToolBear JWT secret (HS256) from the old deployment.
      """)

  config :lilium_chat, :jwt, secret: jwt_secret

  # S3 / SeaweedFS credentials for SigV4 presign (spec §6.2).
  # Read lazily by the presign module; not required to boot.
