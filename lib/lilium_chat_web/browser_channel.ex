  ...
  * newer → re-check `channel_members` (SoT) before delivering; update
  ...
  def handle_in("command", payload, socket) do
    case Frames.parse_command(payload) do
      {:ok, {command, command_id, _channel_id, cmd_payload}} ->
        handle_command(command, command_id, cmd_payload, socket)

      {:error, reason} ->
        {:reply, {:error, Frames.command_error(payload["command_id"] || "unknown", error_from(reason))}, socket}
    end
  end
  ...
  defp handle_command("session.live_start", command_id, _payload, socket) do
    case start_live(socket) do
      {:ok, socket, count} ->
        ack = Frames.command_ack("session.live_start", command_id, %{
  ...
  defp handle_command("session.heartbeat", command_id, _payload, socket) do
    if socket.assigns[:live?] do
      ack = Frames.command_ack("session.heartbeat", command_id, %{
        "session_id" => socket.assigns[:session_id],
        "lease_expires_at" => lease_expiry()
      })
      {:reply, {:ok, ack}, socket}
  ...
  defp handle_command(command, command_id, _payload, socket) do
  ...
  defp error_from(code) do
    api_error = Errors.new(code)
